...
pub fn calculate_sticky_falls_from_rows(board: &Board, mut rows: Vec<i32>) -> Vec<(Point, i32)> {
    rows.sort();
    ...
    calculate_sticky_falls(board, points)    
}

pub fn calculate_sticky_falls(board: &Board, mut visit: Vec<Point>) -> Vec<(Point, i32)> {
